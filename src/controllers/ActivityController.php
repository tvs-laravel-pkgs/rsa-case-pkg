			...
			foreach ($activities as $activity_key => $activity) {
				...
				if (Entrust::can('export-own-activities') || Entrust::can('export-own-rm-asp-activities') || Entrust::can('export-own-zm-asp-activities')) {
					...
					$activity_details_data[] = [
						...
						$activity->remarks != NULL ? strip_tags($activity->remarks) : '',
						$activity->manual_uploading_remarks != NULL ? $activity->manual_uploading_remarks : '',
						$activity->general_remarks != NULL ? strip_tags($activity->general_remarks) : '',
						$activity->bo_comments != NULL ? $activity->bo_comments : '',
						$activity->deduction_reason != NULL ? $activity->deduction_reason : '',
						$activity->defer_reason != NULL ? strip_tags($activity->defer_reason) : '',
						$activity->asp_resolve_comments != NULL ? strip_tags($activity->asp_resolve_comments) : '',
