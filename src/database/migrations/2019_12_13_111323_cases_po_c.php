...
class CasesPoC extends Migration {
	...
	public function up() {
		Schema::create('cases', function (Blueprint $table) {
			...
			$table->foreign('cancel_reason_id')->references('id')->on('case_cancel_reasons')->onDelete('SET NULL')->onUpdate('cascade');
			$table->datetime('data_filled_date');
			$table->string('vehicle_registration_number', 24);
			$table->string('vin_no', 24)->nullable();
			$table->unsignedInteger('eligibility_type_id')->nullable();
			$table->foreign('eligibility_type_id')->references('id')->on('entities')->onDelete('SET NULL')->onUpdate('cascade');
			...
			$table->foreign('policy_id')->references('id')->on('policies')->onDelete('SET NULL')->onUpdate('cascade');
